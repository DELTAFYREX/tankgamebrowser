...
if (window.innerWidth < 980) {
    ...
    var tinyscreen = true;
    if (window.innerWidth < 565) var tinierscreen = true;
...
function switchmenu() {
    clicked = !clicked;
    if (clicked) {
        $("#startMenuSlidingTrigger").html('Close Details');
        $("#controls").css({ position: "relative", width: "100%", opacity: "100%", "margin-left": "-25px" });
        $("#browser").css({ position: "relative", width: "0%", opacity: "0%", "margin-right": "0px" });
    } else {
        $("#startMenuSlidingTrigger").html('Open Details');
        $("#controls").css({ position: "relative", width: "0%", opacity: "0%", "margin-left": "0px" });
        $("#browser").css({ position: "relative", width: "100%", opacity: "100%", "margin-right": "-15px" });
    }
}
...
class DOMServerListItem {
    constructor (hasApp, hasBrowser, secure, ip, motdDefault, index) {
        ...
        this.textContainer.append(this.name, tinierscreen ? "" : this.description);
...
fetch(`${location.protocol}//${location.host}/public/otherServers.json`).then(x => x.json()).then(fetchedServers => {
    ...
    if (!tinyscreen) getSelected().motdSocket.addEventListener('message', () => getSelected().element.select(), { once: true });
});
